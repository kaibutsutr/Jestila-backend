...
(function ($, window, document, undefined) {
  ...
  GlassCase.defaults = {
    ...
    //THUMBS AREA
    thumbsPosition: "top", // Default position of thumbs. Position is relative to the image display. Can take the values: top; bottom
  ...
  GlassCase.prototype = {
    init: function (options) {
      ...
      var sVT =
        gcBase.config.thumbsPosition == "right" ||
        gcBase.config.thumbsPosition == "left"
          ? "-vt"
          : "";

      var ctntThumbsPrevNext =
        "<div class='gc-thumbs-area-prev'><div class='gc-icon gc-icon-prev" +
        sVT +
        "'></div></div>" +
        "<div class='gc-thumbs-area-next'><div class='gc-icon gc-icon-next" +
        sVT +
        "'></div></div>";
      ...
      // Plugin variables
      // Loading
      gcBase.gcLoadingClass =
        Modernizr.csstransforms == true ? "gc-loading3" : "gc-loading";
      ...
      gcBase.gcDisplayContainer = gcBase.gcDisplayArea.find(
        ".gc-display-container"
      );
      gcBase.gcDisplayDisplay = gcBase.gcDisplayContainer.find(
        ".gc-display-display"
      );
      ...
      gcBase.gcOverlayGContainer = gcBase.gcOverlayArea.find(
        ".gc-overlay-gcontainer"
      );
      gcBase.gcOverlayContainer = gcBase.gcOverlayArea.find(
        ".gc-overlay-container"
      );
      gcBase.gcOverlayContainerDisplay = gcBase.gcOverlayContainer.find(
        ".gc-overlay-container-display"
      );
      gcBase.gcOverlayDisplay = gcBase.gcOverlayContainer.find(
        ".gc-overlay-display"
      );
      gcBase.gcOverlayPrevious = gcBase.gcOverlayArea.find(".gc-icon-prev");
      gcBase.gcOverlayNext = gcBase.gcOverlayArea.find(".gc-icon-next");
      gcBase.gcOverlayClose = gcBase.gcOverlayArea.find(".gc-icon-close");
      gcBase.gcOverlayEnlarge = gcBase.gcOverlayArea
        .find(".gc-icon-enlarge")
        .hide();
      gcBase.gcOverlayCompress = gcBase.gcOverlayArea
        .find(".gc-icon-compress")
        .hide();
      // Thumbs: Area, Ul, Li, AreaPrevious, AreaNext, Previous, Next, Img, LiDiv
      gcBase.gcThumbsUl = gcBase.element.find("ul");
      gcBase.gcThumbsUl.wrap(
        "<div class='gc-thumbs-area-main'><div class='gc-thumbs-area'></div></div>"
      );
      ...
      gcBase.gcThumbsAreaPrevious = gcBase.gcThumbsAreaMain.find(
        ".gc-thumbs-area-prev"
      );
      gcBase.gcThumbsPrevious = gcBase.gcThumbsAreaPrevious.find(
        ".gc-icon-prev" + sVT
      );
      gcBase.gcThumbsAreaNext = gcBase.gcThumbsAreaMain.find(
        ".gc-thumbs-area-next"
      );
      ...
      if (gcBase.config.isZCapEnabled === true) {
        gcBase.gcCaption = $(
          '<div class="gc-caption-container"><div></div></div>'
        );
        gcBase.gcCaptionDisplay = gcBase.gcCaption.find("div");

        if (gcBase.config.zoomPosition === "inner")
          gcBase.config.capZType = "in";

        cssClass =
          "gc-caption-" + gcBase.config.capZType + gcBase.config.capZPos;
        if (
          $.inArray(cssClass, [
            "gc-caption-outtop",
          ...
          cssClass =
            "gc-caption-" +
            gcBase._defaults.capZType +
            gcBase._defaults.capZPos;
        }
      ...
      if (
        gcBase.config.isDownloadEnabled == false ||
        gcBase.supportCanvas == false
      ) {
      ...
      if (
        isNaN(gcBase.config.firstThumbSelected) == false &&
        parseFloat(gcBase.config.firstThumbSelected) > -1 &&
        parseFloat(gcBase.config.firstThumbSelected) <=
          gcBase.gcThumbsLi.length - 1
      ) {
      ...
      gcBase.currentSlide = Math.floor(
        gcBase.current / gcBase.config.nrThumbsPerRow
      );
      gcBase.old = 0;
      ...
      gcBase.gcDisplayDisplay
        .attr("src", gcBase.gcThumbsImg.eq(gcBase.current).attr("src"))
        .attr("alt", altTxt);
      gcBase.setup();
          ...
          $("<img/>")
            .on("load", function () {
              var lWidth = this.width,
                ...
                index = gcBase.gcThumbsLi
                  .find("img[src*='" + $(this).attr("src") + "']")
                  .parents("li")
                  .index();
            ...
            .on("error", function () {
              ...
              if (Modernizr.svg) {
                ...
                gcBase.gcThumbsImg
                  .eq(index)
                  .attr("src", "data:image/svg+xml;base64," + iEDB64);
              }
              gcBase.processThumbImage(index);
              if (++countLoadedImages === countTotalImages) {
                dfd.resolve();
    ...
    setup: function () {
      ...
      if (
        (gcBase.config.thumbsPosition == "right" ||
          gcBase.config.thumbsPosition == "left") &&
        gcBase.config.isOneThumbShown == false &&
        gcBase.gcThumbsLi.length > 1
      ) {
        var liMargin = parseFloat(gcBase.gcThumbsLi.css("margin-bottom")),
          heightLi =
            parseFloat(gcBase.config.heightDisplay) /
              gcBase.config.nrThumbsPerRow -
            ((gcBase.config.nrThumbsPerRow - 1) * liMargin) /
              gcBase.config.nrThumbsPerRow,
          ratio = gcBase.config.widthDisplay / gcBase.config.heightDisplay,
          widthLi = heightLi * ratio;
        var wE =
          widthLi +
          gcBase.config.thumbsMargin +
          parseFloat(gcBase.config.widthDisplay);

        gcBase.widthDisplayPerc = Math.round(
          (gcBase.config.widthDisplay * 100) / wE
        );

        gcWidth =
          gcBase.element.parent().width() > wE
            ? wE
            : gcBase.element.parent().width();
      } else {
        gcWidth =
          gcBase.element.parent().width() > gcBase.config.widthDisplay
            ? gcBase.config.widthDisplay
            : gcBase.element.parent().width();
      }
      ...
      if (gcBase.gcTotalThumbsImg == 1) {
        ...
        gcBase.gcOverlayPrevious.css(
          "margin-top",
          -(gcBase.gcOverlayPrevious.outerHeight() / 2)
        );
        gcBase.gcOverlayNext.css(
          "margin-top",
          -(gcBase.gcOverlayNext.outerHeight() / 2)
        );
      }
      // COMPONENT
      if (
        gcBase.config.thumbsPosition == "top" ||
        gcBase.config.thumbsPosition == "bottom"
      ) {
        var hThumbs =
          gcBase.config.isOneThumbShown == false
            ? 0
            : gcBase.gcThumbsArea.outerHeight();
        gcBase.element.css({
          height:
            hThumbs +
            gcBase.gcDisplayArea.outerHeight() +
            parseFloat(gcBase.config.thumbsMargin),
        });
      } else {
        var wThumbs =
          gcBase.config.isOneThumbShown == false
            ? 0
            : gcBase.gcThumbsArea.outerWidth();
        gcBase.element.css({
          width:
            wThumbs +
            gcBase.gcDisplayArea.outerWidth() +
            parseFloat(gcBase.config.thumbsMargin),
        });
    ...
    setupDisplayArea: function () {
      ...
      var currentDisplayAreaWidth,
        currentDisplayAreaHeight,
        nextDisplayAreaWidth,
        nextDisplayAreaHeight,
      ...
      nextDisplayAreaWidth =
        (gcBase.widthDisplayPerc * gcBase.element.outerWidth()) / 100;
      ...
      gcBase.gcDisplayPrevious.css(
        "margin-top",
        -(gcBase.gcDisplayPrevious.outerHeight() / 2)
      );
      gcBase.gcDisplayNext.css(
        "margin-top",
        -(gcBase.gcDisplayNext.outerHeight() / 2)
      );
    ...
    setupDisplayDisplay: function () {
      ...
      var widthRatio =
          gcBase.gcDisplayContainer.outerWidth() /
          gcBase.gcImageData[gcBase.current].width,
        heightRatio =
          gcBase.gcDisplayContainer.outerHeight() /
          gcBase.gcImageData[gcBase.current].height,
        ratio,
        ddWidth,
        ddHeight;

      if (widthRatio < 1 || heightRatio < 1) {
        gcBase.config.isZoomEnabled === true
        ...
        // In case that the image's width and height are smaller than the container's windth and height
      ...
      var borderVal =
          parseFloat(gcBase.gcDisplayArea.css("border-left-width")) * 2,
        paddingVal = parseFloat(gcBase.gcDisplayArea.css("padding-top")) * 2;

      var percMarginLeft =
          ((gcBase.gcDisplayContainer.outerWidth() / 2) * 100) /
          (gcBase.gcDisplayArea.outerWidth() - borderVal - paddingVal),
        percMarginTop =
          ((gcBase.gcDisplayContainer.outerHeight() / 2) * 100) /
          (gcBase.gcDisplayArea.outerWidth() - borderVal - paddingVal);
    ...
    setupZoom: function () {
      ...
      gcBase.gcZoomDisplay
        .attr("src", gcBase.gcDisplayDisplay.attr("src"))
      ...
      var borderVal = parseFloat(gcBase.gcZoom.css("border-left-width")) * 2,
        paddingVal = parseFloat(gcBase.gcDisplayArea.css("padding-top")) * 2,
        zoomWidth =
          gcBase.config.zoomPosition == "inner"
            ? paddingVal
            : borderVal + paddingVal,
        zoomHeight =
          gcBase.config.zoomPosition == "inner"
            ? paddingVal
            : borderVal + paddingVal;
      ...
      for (var i = 0; i < 2; i++) {
        if (gcBase.config.isZoomDiffWH && gcBase.config.zoomWidth > 0) {
          zoomWidth +=
            parseFloat(gcBase.config.zoomWidth) <
            gcBase.gcImageData[gcBase.current].width
              ? parseFloat(gcBase.config.zoomWidth)
              : gcBase.gcImageData[gcBase.current].width;
        } else {
        ...
        if (gcBase.config.isZoomDiffWH && gcBase.config.zoomHeight > 0) {
          zoomHeight +=
            parseFloat(gcBase.config.zoomHeight) <
            gcBase.gcImageData[gcBase.current].height
              ? parseFloat(gcBase.config.zoomHeight)
              : gcBase.gcImageData[gcBase.current].height;
        } else {
        ...
        if (
          gcBase.config.autoInnerZoom == true &&
          gcBase.config.zoomPosition != "inner"
        ) {
      ...
      if (gcBase.config.isZCapEnabled === true) {
        ...
        capTxt === undefined
          ? gcBase.gcCaption.hide()
          : (gcBase.gcCaption.show(),
            gcBase.gcCaptionDisplay.empty().append(capTxt));
        var cssClass;

        if (gcBase.isAutoInnerZooming === true) {
          if (gcBase.config.capZType === "out") {
            gcBase.gcCaption.removeClass(
              "gc-caption-outtop gc-caption-outbottom"
            );

            cssClass =
              gcBase.config.capZPos === "top"
                ? "gc-caption-intop"
                : "gc-caption-inbottom";
          ...
          if (
            gcBase.config.capZType === "out" &&
            (gcBase.gcCaption.hasClass("gc-caption-intop") ||
              gcBase.gcCaption.hasClass("gc-caption-inbottom"))
          ) {
            gcBase.gcCaption.removeClass(
    ...
    setupZoomPos: function () {
      ...
      if (
        ...
        gcBase.gcZoom
          .appendTo(gcBase.gcDisplayContainer)
          .addClass("gc-zoom-inner");
      } else {
        gcBase.gcZoom.appendTo(gcBase.element).removeClass("gc-zoom-inner");
        ...
        var topZ =
          gcBase.config.zoomAlignment == "displayArea"
            ? 0
            : gcBase.gcDisplayContainer.position().top +
    ...
    setupLens: function () {
      ...
      var percZoomWidth = Math.round(
        (gcBase.gcZoomContainer.outerWidth() /
          gcBase.gcImageData[gcBase.current].width) *
          100
      );
      var valueLensW = Math.round(
        (gcBase.gcDisplayDisplay.outerWidth() * percZoomWidth) / 100
      );

      var percZoomHeight = Math.round(
        (gcBase.gcZoomContainer.outerHeight() /
          gcBase.gcImageData[gcBase.current].height) *
          100
      );
      var valueLensH = Math.round(
        (gcBase.gcDisplayDisplay.outerHeight() * percZoomHeight) / 100
      );
    ...
    addLoader: function (obj) {
      //obj - the object that will contain the loader
      var gcBase = this;
    ...
    setupThumbImg: function (obj, index) {
      ...
      gcBase.gcThumbsImg[index].width = Math.ceil(
        gcBase.gcImageData[index].width * ratioImg,
        10
      );
      gcBase.gcThumbsImg[index].height = Math.ceil(
        gcBase.gcImageData[index].height * ratioImg,
        10
      );

      var percMarginLeft =
          ((gcBase.gcThumbsImg.eq(index).outerWidth() / 2) * 100) /
          gcBase.gcThumbsLiDiv.outerWidth(),
        percMarginTop =
          ((gcBase.gcThumbsImg.eq(index).outerHeight() / 2) * 100) /
          gcBase.gcThumbsLiDiv.outerWidth();
    ...
    setupThumbsTB: function () {
      ...
      var liMarginRight = parseFloat(gcBase.gcThumbsLi.css("margin-right")),
        ratio = gcBase.config.widthDisplay / gcBase.config.heightDisplay,
        widthLi =
          gcBase.gcThumbsArea.outerWidth() / gcBase.config.nrThumbsPerRow -
          ((gcBase.config.nrThumbsPerRow - 1) * liMarginRight) /
            gcBase.config.nrThumbsPerRow,
      ...
      if (gcBase.config.isThumbsOneRow == true) {
        widthLiPerc =
          (widthLi * 100) /
          ((widthLi + liMarginRight) * gcBase.gcThumbsLi.length -
            liMarginRight);
      } else {
      ...
      if (gcBase.config.isThumbsOneRow == true) {
        ...
        gcBase.gcThumbsUl
          .find(":nth-child(" + gcBase.config.nrThumbsPerRow + "n)")
          .css("margin-right", 0);
        gcBase.gcThumbsUl
          .find(
            ":nth-child(n +" +
              (parseFloat(gcBase.config.nrThumbsPerRow) + 1) +
              ")"
          )
          .css("margin-top", liMarginRight + "px");
      }
      if (gcBase.config.isThumbsOneRow == true) {
        gcBase.gcThumbsUl.css({
          width: Math.ceil(
            widthLi * gcBase.gcThumbsLi.length +
              (gcBase.gcThumbsLi.length - 1) * liMarginRight
          ),
        ...
        var totalRows = Math.ceil(
          gcBase.gcThumbsLi.length / gcBase.config.nrThumbsPerRow
        );
        var lHeight = Math.ceil(
          heightLi * totalRows + liMarginRight * (totalRows - 1)
        );
      ...
      if (gcBase.config.isThumbsOneRow == true) {
        ...
        gcBase.gcThumbsPrevious.css(
          "margin-top",
          -gcBase.gcThumbsPrevious.outerHeight() / 2
        );
        gcBase.gcThumbsAreaNext.removeClass("gc-hide");
        gcBase.gcThumbsNext.css(
          "margin-top",
          -gcBase.gcThumbsNext.outerHeight() / 2
        );
      ...
      if (gcBase.iOS) {
        var brwLiWidth = gcBase.gcThumbsLi.outerWidth(),
          brwDiff =
            gcBase.gcThumbsArea.outerWidth() -
            (brwLiWidth * gcBase.config.nrThumbsPerRow +
              (gcBase.config.nrThumbsPerRow - 1) * liMarginRight);
        gcBase.gcThumbsUl
          .find(":nth-child(" + gcBase.config.nrThumbsPerRow + "n)")
          .css("width", brwLiWidth + brwDiff);
      }
    },
    setupThumbsLR: function () {
      ...
      var liMargin = parseFloat(gcBase.gcThumbsLi.css("margin-bottom")),
        ratio = gcBase.config.widthDisplay / gcBase.config.heightDisplay,
        heightLi =
          gcBase.gcThumbsArea.outerHeight() / gcBase.config.nrThumbsPerRow -
          ((gcBase.config.nrThumbsPerRow - 1) * liMargin) /
            gcBase.config.nrThumbsPerRow,
        widthLi = heightLi * ratio,
        heightLiPerc;
      heightLiPerc =
        (heightLi * 100) /
        ((heightLi + liMargin) * gcBase.gcThumbsLi.length - liMargin);
      ...
      gcBase.gcThumbsPrevious.css(
        "margin-left",
        -gcBase.gcThumbsPrevious.outerWidth() / 2
      );
      gcBase.gcThumbsAreaNext.removeClass("gc-hide");
      gcBase.gcThumbsNext.css(
        "margin-left",
        -gcBase.gcThumbsNext.outerWidth() / 2
      );
      ...
      if (gcBase.iOS) {
        ...
        var brwDiff =
          gcBase.gcThumbsArea.outerHeight() -
          (brwLiHeight * gcBase.config.nrThumbsPerRow +
            (gcBase.config.nrThumbsPerRow - 1) * liMargin);
        gcBase.gcThumbsUl
          .find(":nth-child(" + gcBase.config.nrThumbsPerRow + "n)")
          .css("height", brwLiHeight + brwDiff);
      }
    },
    setupSlider: function () {
      ...
      if (
        gcBase.currentSlide ==
        Math.floor(
          (gcBase.gcThumbsLi.length - 1) / gcBase.config.nrThumbsPerRow
        )
      ) {
    ...
    update: function () {
      ...
      if (gcBase.config.colorActiveThumb != -1) {
        gcBase.element
          .find(".gc-active")
          .css("border-color", gcBase.config.colorActiveThumb);
      }
      ...
      gcBase.gcDisplayDisplay
        .attr(
          "src",
          gcBase.gcThumbsLi.eq(gcBase.current).find("img").attr("src")
        )
        .attr("alt", altTxt);
    ...
    nextImage: function () {
      ...
      gcBase.current =
        gcBase.current == gcBase.gcThumbsLi.length - 1 ? 0 : gcBase.current + 1;
      gcBase.slide("true", "");
      gcBase.animateImage();
    },
    previousImage: function () {
      ...
      gcBase.old = gcBase.current;
      gcBase.current =
        gcBase.current == 0 ? gcBase.gcThumbsLi.length - 1 : gcBase.current - 1;
      gcBase.slide("true", "");
      gcBase.animateImage();
    },
    slide: function (isImageChange, slideChange) {
      //isImageChange: true || false; slideChange:   previous || next
      var gcBase = this;

      if (
        gcBase.config.isThumbsOneRow == false &&
        (gcBase.config.thumbsPosition == "bottom" ||
          gcBase.config.thumbsPosition == "top")
      ) {
        return;
      ...
      if (
        ...
        vMargin =
          gcBase.gcThumbsArea.outerWidth() +
          parseFloat(gcBase.gcThumbsLi.css("margin-right"));
        ...
        vMargin =
          gcBase.gcThumbsArea.outerHeight() +
          parseFloat(gcBase.gcThumbsLi.css("margin-bottom"));
        gcBase.gcThumbsUl.animate(
    ...
    mousemoveHandler: function (event) {
      ...
      if (
        gcBase.config.isSlowZoom == false ||
        (gcBase.config.isSlowZoom == true && event == undefined)
      ) {
      ...
      if (
        gcBase.config.isSlowLens == false ||
        (gcBase.config.isSlowLens == true && event == undefined)
      ) {
        gcBase.gcLens.css({
    ...
    calcMousePos: function (currentMousePos) {
      ...
      if (mouseXRelative + lensWidth / 2 > 0 + imageDisplayWidth) {
        ...
        zoomLeft = -(
          gcBase.gcImageData[gcBase.current].width - gcBase.gcZoom.outerWidth()
        );
      }
    ...
    zoomSlowDown: function () {
      ...
      //2.
      moveZoomPos.left =
        -diffZoomPos.left / (gcBase.config.speedSlowZoom / 100);
    ...
    lensSlowDown: function () {
      ...
      //2.
      moveLensPos.left =
        -diffLensPos.left / (gcBase.config.speedSlowLens / 100);
    ...
    setupOverlay: function () {
      ...
      var isNatSizeSMScr =
        gcBase.gcImageData[gcBase.current].width <=
          gcBase.gcOverlayArea.outerWidth() &&
        gcBase.gcImageData[gcBase.current].height <=
          gcBase.gcOverlayArea.outerHeight();

      gcBase.gcOverlayDisplay
        .attr("src", gcBase.gcDisplayDisplay.attr("src"))
        .attr("alt", gcBase.gcDisplayDisplay.attr("alt"));
    ...
    overlayNatSizes: function () {
      ...
      if (
        gcBase.gcImageData[gcBase.current].height <= hOC &&
    ...
    initEvents: function () {
      ...
      if (gcBase.config.isZoomEnabled === true) {
        gcBase.isMouseEventsOn = true;
        gcBase.gcDisplayContainer
          .on("touchstart.glasscase", $.proxy(gcBase.touchstartDC, gcBase))
          .on("touchmove.glasscase", $.proxy(gcBase.touchmoveDC, gcBase))
          .on("touchend.glasscase", $.proxy(gcBase.touchendDC, gcBase));
        gcBase.gcDisplayContainer
          .on("mousemove.glasscase", $.proxy(gcBase.mousemoveHandler, gcBase))
          .on("mouseenter.glasscase", $.proxy(gcBase.mouseenterHandler, gcBase))
          .on(
            "mouseenter.glasscase",
            $.proxy(gcBase.config.mouseEnterDisplayCB, gcBase)
          )
          .on("mouseleave.glasscase", $.proxy(gcBase.mouseleaveHandler, gcBase))
          .on(
            "mouseleave.glasscase",
            $.proxy(gcBase.config.mouseLeaveDisplayCB, gcBase)
          );
      }
